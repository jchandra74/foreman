...
but then you are the dispatcher, babysitting every build, every review, every merge.
foreman keeps the ticket-sized scope and automates the dispatching, with three guardrails:
...
**3. Optional: the [Codex CLI](https://github.com/openai/codex)**, to put GPT builders and
reviewers in the run via the `codex` argument (see [Model routing](#model-routing)).
...
Claude Code; [Model routing](#model-routing) explains why.
...
Plugin skills are namespaced, so `/foreman:run-tickets` is the form that always resolves.
...
- **`-c mcp_servers={}` and `< /dev/null` on every invocation.** Without the first, the
  run inherits your personal Codex MCP servers, and an unguarded review here wandered into
  a browser session and timed out. Without the second, `codex exec` blocks forever waiting
